...
export class PermissionRequestsService {
  ...
  async createLeavingPermissionRequest(
    studentId: number,
    leavingPermissionRequestDto: CreateLeavingPermissionDto,
  ) {
    ...
    try {
      ...
      const { absenceTimeSlots, reason, justificationDeadline } =
      ...
      const permission = await this.permissionsRepository.save({
        ...
        reason,
        status: PermissionStatus.LEAVE_PERMISSION,
      });
      ...
      return {
        message: 'Leaving permission request created successfully',
      };
    } catch (err) {
